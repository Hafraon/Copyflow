...
export async function generateWithAssistant(request: GenerationRequest): Promise<AssistantResult> {
  const assistantId = process.env.OPENAI_ASSISTANT_UNIVERSAL;
  
  // Try Assistant API first if Assistant ID is configured
  if (assistantId) {
    try {
      console.log('Attempting generation with OpenAI Assistant API...');
      const assistantResult = await tryAssistantAPI(assistantId, request);
      if (assistantResult.success) {
        console.log('✅ Assistant API generation successful');
        return { ...assistantResult, method: 'assistant' };
      }
      console.log('⚠️ Assistant API failed, falling back to Chat Completions');
    } catch (error) {
      console.error('Assistant API error:', error);
      console.log('⚠️ Assistant API error, falling back to Chat Completions');
    }
  } else {
    console.log('No Assistant ID configured, using Chat Completions directly');
  }
